...
def filter_data(df: pd.DataFrame, country: str) -> pd.DataFrame:
    ...
    df_sub.loc[:, 'total_vaccinations'] = (
        df_sub['people_vaccinated'] + df_sub['people_fully_vaccinated'])
    ...
    df_sub.loc[:, 'vaccine'] = [
        VACCINE_MAPPER.get(vac, 'UNKNOWN') for vac in df_sub['Vaccine']]
...
def aggregate_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_agg = (df
        .groupby(['location', 'date', 'source_url'])
        .agg({
